...
Puis, on a cherché des bons modèles avec plusieures méthodes: 

* une méthode itératif en interpretant les graphiques des ACF et PACF et puis en examinant si les résidus sont un bruit blanc et les coéfficients sont significatifs
...
À l'aide de l'AIC et la méthode de coins on a finalement choisi un $\text{SARIMA}_{12}((1,1,2)(0,1,1))$, avec lequelle on a fait la prévision pour l'année 1971.
...
C'est vraisemblable qu'il y a aussi une saisonnalité annuelle, c'est à dire de période 12 car les données sont mensuelles.
...
d = sqrt(miles)
BoxCox_diag(d, main = TeX("$\\sqrt{ X_t}$"))
```
Les points de la série originale, en haut à la gauche, se trouvent un peu loin sous la diagonale, et ils sont d'un coup aussi déjà proche de l'axe.
...
Pour comparaison, les données originales et les transformations sont plotées ci-dessous, standardisées.
```{r echo=FALSE}
...
Afin de reducer cette propriété nous prenons le logarithme népérien (variances: première moitié $`r round(var(head(log(miles),length(miles)/2)), 3)`$ et 2nd $`r round(var(tail(log(miles),length(miles)/2)), 3)`$).
...
On voit bizarrement que les données $\log(X_t)$ sont déja stationnaires:
```{r, echo = TRUE}
...
```
On a fait un Durbin-Watson Test afin de savoir si l'autocorrélation des erreurs estimées d'un modèle linéare `log(miles) ~ date` est égal à 0, qui est rejetée carrément avec une valeur p de $`r round(dw, 5)`$. 
Il y aura sûrement des autres assumptions blessées pour faire un GLM, par exemple l'égalité des variances et l'indépendence des valeurs et des erreurs.
...
Peut-être c'est un modèl additif, dans lequel on modéliser plusieurs componants lesquelles, en somme, égalent la série d'origine :
...
##### traiter la tendance en différenciant
...
Il n'y a aucun coéfficient significatif, le coéfficient de l'ordonnée à l'origine est presque nul.

Laquelle méthode choisir ? Généralment différencier est mieux.
...
Dans le suivant on va essayer de entraîner le bon modèle, bien sachant qu'il y plusieurs modèles qui peuvent apparaître significatifs.
Après qu'on a transformé les données on a également déjà différencié, et donc en differenciant on a aussi déja trouver que $d=1$ et $D=1$.
...
\end{aligned}
$$
Autrement dit, une $\text{SARIMA}_{12}[(p= 11,d= 1,q= 0),(P= 0,D= 1,Q= 1)]$. On va tenter ce modèle.
...
```
Il semble qu'il ait trop d'estimateurs, aucun est significant.
...
Cette méthode itérative peux bien servir à trouver le bon modèle.
...
Le Ljung-Box-Test rejete avec une p-valeur `r Box.test(residuals(modele5), type = "Ljung-Box")$p.value` l'hypothese que les résidus ne soient pas un bruit blanc.
#### Modèle 6
...
Le Ljung-Box-Test donne le p-valeur `r Box.test(residuals(modele6), type = "Ljung-Box")$p.value`.
...
Pour comparer la qualité des prévisions on peut utiliser le MSE (mean squared error), le plus faible le meilleur :
...
En bleu les données la prévision sur `miles.test`, et la courbe noir sont les vraies valeurs :
...
```
MSE, c'est la diffèrence en carré entre les deux courbes.
...
Ici, le model2 gagne ! (Pour model21 on 
...
coeftest(modele5)
```
$\theta_1$ n'est pas significatif. On a aussi estimé le modele6 sans ce paramètre, qui s'a averé cependant comme pire que ce modèle-là en ce qui concerne AIC et MSE.

$\theta_2$ est significatif de hauteur de $-0.63$, $\theta_{12}$ est significatif de hauteur de $-0.54$.
Cette partie moyenne mobile indique que les erreurs aléatoires ont un influence pour $X_t$ même avec un decalage de 2 et 12. Le decalage de 12 est facilement expliqué, comme les saisons sont une facteur qui se trouve dans les erreurs. 
L'influence du paramètre $\theta_2$ n'est pas aussi grande, est veut dire que, s'il y a un choc qui n'est pas endogène, il va augmenter la série après deux mois encore dans la même direction d'environ 63%.

$\phi_1$ est significatif de hauteur de $-0.49$.
C'est-à-dire que si la série a augmenté, après un mois elle va garder presque la moitié de cette augmentation en moyenne, après deux un quart et ainsi de suite.

### Les résidus

En regardant les résidus on observe qu'il semblent bien stationnaires.
Apparemment ils sont indépendants, il y a que $\hat r(19)$ qui est significatif (le circle rouge). Je dis que c'est dû au hasard.
On trouve que $E(\varepsilon_t\varepsilon_s)=0$, également on a déjà fait un teste portmanteau, on peux dire que c'est un bruit blanc.

Alors je constate qu'il ne faut pas ajouter encore des paramètres.
...
plot(residuals(modele5))
acf(residuals(modele5), lag.max = 25, ci.type = "ma")
mtext("Les résidus de modèle 5",3,las=1)
pacf(residuals(modele5, lag.max = 25))
symbols(c(19/12),c(-.22),circles=c(0.1),inches=F, add=T, fg= "red")
```
...
```
On re-transforme la prévision de  $log(X_t)$ avec la fonction exponentielle de base $e$.
```{r}
