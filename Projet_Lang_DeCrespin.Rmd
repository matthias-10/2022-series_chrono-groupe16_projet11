...
* des coéfficients sigificatifs
* une forme canonique ainsi que restent que des bruits blancs (Box-Jenkins-Test)
* des $\varepsilon_t$ non corrélés.
...
$$(1-\phi_1B-\phi_2B^2-\phi_3B^3-\phi_4B^4-\phi_5B^5-\phi_6B^6-\phi_7B^7-\phi_8B^8-\phi_9B^9-\phi_{10}B^{10}-\phi_{11}B^{11})(1-\phi_{12}B^{12}) (1-B)(1-B^{12}) \log(X_t) = (1-\theta_{12}B^{12})\epsilon_t$$
...
$$(1-\phi_1B-\phi_2B^2-\phi_4B^4-\phi_{10}B^{10}-\phi_{11}B^{11})(1-\phi_{12}B^{12}) (1-B)(1-B^{12}) \log(X_t) = (1-\theta_{12}B^{12})\epsilon_t$$
...
SARIMA(11,1,2)(0,1,1)[12] :

$$(1-\phi_1B-\phi_2B^2-\phi_4B^4-\phi_{10}B^{10}-\phi_{11}B^{11})(1-\phi_{12}B^{12}) (1-B)(1-B^{12}) \log(X_t) =
...
```{r model 3}
modele3 <- arima(log(miles), order = c(10,1,2), seasonal=list(order = c(0,1,1), perdiod=12))
...
### Modèle 4
$$(1-\phi_1B-\phi_2B^2)(1-B)(1-B^{12}) \log(X_t) = 
   (1-\theta_1B-\theta_{12}B^{12})\epsilon_t$$
Plus parcimonieux que modèle 3, avec un MA(1):

```{r  model 4}
modele4 <- arima(log(miles), order = c(0,1,1), seasonal=list(order = c(0,1,1), perdiod=12))
sarima_comparai <- arima(mdiff12diff1, order = c(0,0,2), seasonal=list(order = c(0,0,1), perdiod=12)) #devrait être le même ?
summary(modele4)
coeftest(modele4) #https://stackoverflow.com/questions/43826952/how-can-i-get-the-t-statistics-for-the-results-of-an-ar1-model-in-r
```
