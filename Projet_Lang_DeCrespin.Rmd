...
```{r}
dw = dwtest(miles ~ date, data = donnees)
```
On a fait un Durbin-Watson Test: $H_0$, que l'autocorrelation des erreurs estimées d'un modèle linéare `miles ~ date` est égal à 0, est rejetée avec une valeur p de `r dw$p.value`. 
Clairement on rejète $H_0$.
Il y aura sûrement des autres assumptions blessées pour faire un GLM, par exemple l'égalité des variances et l'indépance des valeurs et des erreurs.
...
```{r delta}
mdiff12 = diff(miles,12)
#tsdisplay(mdiff)
# tsdisplay(mdiff12,ci.type="ma", main = "(1-B^12)X_t")
# = pacf(mdiff, ci.type="ma")
# =  acf(mdiff, ci.type="ma")
adf.test(mdiff12)

plot(mdiff12)
fit.12 <- lm(mdiff12 ~ tail(donnees$date,84))
abline(fit.12, col="red")
summary(fit.12)
```
...
```{r delta^2}
mdiff12diff1 = diff(mdiff12, lag=1, difference = 1)
