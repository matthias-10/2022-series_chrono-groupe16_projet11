...
Le meilleur sera de soit une transformation de Box et Cox, soit qu'on affirme qu'ils sont déjà stationnaire.
Mais comme vu, le logharithme népérien fait aussi en sorte que les variances soient égales, même si Box et Cox seraient mieux.
On va l'utiliser pour son aise dans la suite.

On voit que les données sont déja stationnaire, même si on peut observer toujours une tendance et une saisonnalité.
Ce fait est sûrement dû au petit nombre de données.
```{r}
