...
```{r echo=TRUE}
DELTA  = function(i,j, data=log(miles))
{
...
minArma #ARMA(1,1) ou ARMA(1,3) au maximum
```
```{r echo=FALSE}
library(ggplot2)
library(reshape2)
...
ggplot(minArma2, aes(x = p, y = q)) +
    ...
    scale_fill_gradientn(colours=c("red","red","white","green","green"),
                         values=c(0,0.01,0.05,0.25,1),
                         guide = "none", na.value = "white") +
    coord_equal() +
    labs(title = TeX("$\\Delta(p,q)$"))
```
